...
class StravaAuthManager: ObservableObject {
    ...
    func authenticateWithStrava() {
        let appOAuthUrlStravaScheme = URL(
            string: "strava://oauth/mobile/authorize?client_id=\(clientID)&redirect_uri=\(redirectURI.addingPercentEncoding(withAllowedCharacters: .urlQueryAllowed) ?? "")&response_type=code&approval_prompt=auto&scope=\(scope)"
        )!
